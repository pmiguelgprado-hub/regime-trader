...
| # | Finding | Disposition |
|---|---------|-------------|
| **F1** | **No track-record recorder.** `book_snapshot.json` is overwritten each run; nothing accumulated a daily book / EW-S&P500 / SPY NAV series — the gate would have had nothing to evaluate at month 12. | **BUILT** `core/track_record.py` + `main --record-track` + daily plist (23:00). Benchmarks = investable ETF buy-and-hold NAVs: **SPY** (cap-weight) + **RSP** (S&P 500 Equal Weight) — net-of-fee, no synthetic-turnover-cost bias (a hand-rolled gross daily-rebal EW would be unfairly hard on the net book NAV). Verified live (RSP fetch distinct from SPY). |
